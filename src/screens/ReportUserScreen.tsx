...
type ReportUserNavProp = StackNavigationProp<RootStackParamList, 'ReportUser'>;
...
const REASONS: ReasonOption[] = [
  { key: 'harassment', label: 'Assédio ou comportamento agressivo' },
  { key: 'inappropriate', label: 'Conteúdo inapropriado' },
  { key: 'spam',         label: 'Spam ou perfil falso' },
  { key: 'fake',         label: 'Identidade falsa' },
  { key: 'other',        label: 'Outro' },
];

const ReportUserScreen: React.FC = () => {
  const navigation = useNavigation<ReportUserNavProp>();
  ...
  useEffect(() => {
    firestoreService.users.get(userId).then(user => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.reportingLabel}>Você está reportando o usuário:</Text>
        <View style={styles.targetCard}>
          {loadingUser
            ? <ActivityIndicator size="small" color="#999" />
            : <Text style={styles.targetName}>{userName}</Text>
          }
        </View>
...
const styles = StyleSheet.create({
  ...
  reasonOptionSelected: { borderColor: '#FF6B35', backgroundColor: '#FFF8F6' },
  reasonLabel: { fontSize: 16, color: '#1A1A1A', flex: 1, marginRight: 8 },
